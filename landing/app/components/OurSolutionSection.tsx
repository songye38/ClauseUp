function CriteriaCard({
    ...
    labelBg: string
    labelText: string
    labelTextColor?: string // optional
    title: string
    result: string
    textColor: string
}) {
    return (
        <div className={`rounded-lg p-6 ${bg}`}>
            ...
            <div className="my-4">
                <span
                    className={`inline-block rounded px-3 py-1 text-[14px] font-semibold ${labelTextColor || 'text-white'} ${labelBg}`}
                >
                    {labelText}
                </span>
            </div>
...
추가 작업으로 보고 별도 협의합니다"
                ...
                <CriteriaCard
                    ...
                    labelBg="bg-[#F8F844]"
                    labelText="👇🏻 ClauseUp에서는 이렇게 바뀝니다"
                    labelTextColor="text-black"  // <- 여기 추가
